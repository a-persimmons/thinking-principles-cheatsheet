// v2.1 engineering detail augmentation — bilingual Trigger / Checklist / Bad Case / Engineering Pattern
(() => {
  ...
  function enrichDetail(){
    ...
    wrapper.innerHTML = `
      <div class="engineering-trigger">
        ...
        <div class="engineering-trigger-copy">${esc(triggerOf(p))}</div>
      </div>
      <div class="engineering-grid">
        <section class="detail-card engineering-checklist-card">
          <h2>${isEN()?'Engineering checklist':'工程检查清单'}</h2>
          <ul class="engineering-checklist">${checklistOf(p).map(x=>`<li><span class="check-box">□</span><span>${esc(x)}</span></li>`).join('')}</ul>
        </section>
      ...
      <section class="detail-card engineering-pattern-card">
        <div class="engineering-pattern-head"><h2>${isEN()?'Engineering pattern':'工程模式'}</h2><span>${esc(typeOf(p)||'Engineering')}</span></div>
        <code>${esc(patternOf(p)||'—')}</code>
      </section>`;
